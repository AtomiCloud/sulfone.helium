...
CyanEngine.StartExtension(
    args,
    async Task<Cyan> (inquirer, determinism, input) =>
    {
        ...
        if (p.Length > 0 && p[0].Name == "default")
        {
            ...
            if (c?["Variables"]?["bestlang"] != null)
            {
                ...
                else
                {
                    ...
                    var deposit = await inquirer.Text(
                        new TextQ()
                        {
                    ...
                    return new Cyan
                    {
                        Processors = new CyanProcessor[]
                        {
                            ...
                            {
                                Name = "default",
                                Config = new { Variables = new { deposit } },
                                Files = Array.Empty<CyanGlob>(),
                            },
                        },
                        Plugins = Array.Empty<CyanPlugin>(),
