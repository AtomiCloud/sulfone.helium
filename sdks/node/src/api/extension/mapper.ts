...
import type { ExtensionAnswerInput, ExtensionValidateInput } from '../../domain/extension/input.js';
...
class ExtensionMapper {
  public static extensionAnswerToDomain(req: ExtensionAnswerReq): ExtensionAnswerInput {
    const answers: Answer[] = req.answers.map(x => AnswerMapper.toDomain(x));
    const prevAnswers: Answer[] = req.prevAnswers.map(x => AnswerMapper.toDomain(x));
    ...
    const prevExtensionCyans: Record<string, Cyan> = Object.fromEntries(
      Object.entries(req.prevExtensionCyans).map(([key, value]) => [key, CyanMapper.cyanReqToDomain(value)]) satisfies [
        string,
        Cyan,
      ][],
    );
  ...
  public static extensionValidateToDomain(req: ExtensionValidateReq): ExtensionValidateInput {
    return {
